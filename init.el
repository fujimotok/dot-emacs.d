...
"
  ...
  (let ((region-string (buffer-substring-no-properties s e))
        ...
        period)
    ;; 行とスペース区切りの列に分割
    (setq tree (mapcar (lambda (string)
              (split-string string " "))
    ...
    (while tree
      ;; 現在の行の処理をきめて保持
      (setq before-elem (if (not (nth 1 before-elem))
                            (car tree) ;; 前の行がなければ、今の行をそのまま記憶
                          ;; 前の行の時間と期間から、今の行の時間を求めて記憶
                          (setq hour (string-to-number (nth 0 (split-string (nth 0 before-elem) ":"))))
                          (setq min (string-to-number (nth 1 (split-string (nth 0 before-elem) ":"))))
                          (setq period (string-to-number (replace-regexp-in-string "[^0-9]" "" (nth 1 before-elem))))
                          (setq hour (mod (+ hour (/ (+ min period) 60)) 24))
                          (setq min (mod (+ min period) 60))
                          (append (list (format "%02d:%02d" hour min)) (cdr (car tree)))))
      ;; 変更後の結果を保持
      (setq result (append result (list before-elem)))
      ;; treeを1行進める
      (setq tree (cdr tree)))
